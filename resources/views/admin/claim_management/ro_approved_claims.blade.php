...
<title>All RO Approved Claims :: MyG</title>
@include("admin.include.header")

@include("admin.include.sidebar-menu")
<div class="main-area">
    <h2 class="main-heading">All RO Approved Claims</h2>
    <div class="dash-all">
    ...
    <script>
      ...
      $(document).ready(function(){
           ...
           var table = $('#approved-claim-table').DataTable({
               processing: true,
               serverSide: true,
               ajax: "{{ route('ro_approved_claims_list') }}",
               order: ['1', 'DESC'],
               pageLength: 10,
               columns: [
                   { 
                   ...
                   { data: 'RODate', name: 'RODate' },
                   { data: 'TotalAmount', name: 'TotalAmount', className: 'text-right'},
                   { data: 'action', name: 'action', orderable: false, searchable: false}
                    //   render: function(data, type, row) {
                    //     var viewUrl = '/ro_approved_claims_view/' + row.TripClaimID;
